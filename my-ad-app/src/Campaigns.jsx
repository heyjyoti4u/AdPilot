...
// NOTE: primaryImage = brand logo (ab logo clearly dikhega).
// Chahe toh in URLs ko apne hosted logo se replace kar sakti ho.
const INITIAL_DEMO_CAMPAIGNS = [
  {
    ...
    endDate: '2025-12-05',
    primaryImage:
      // ⬇️ Replace with your own Puma logo URL if you want
  ...
  },
  {
    ...
    endDate: '2025-12-10',
    primaryImage:
      // ⬇️ Replace with your own Nike logo URL if you want
      'https://www.freepnglogos.com/uploads/nike-logo-4.jpg',
    caption:
...
const CampaignsDashboard = () => {
  ...
  // (metrics still calculate from DB, but abhi UI me use nahi kar rahe)
    ...
    const interval = setInterval(() => {
      setDemoCampaigns(prev =>
        prev.map(c => ({
          ...
          ads: c.ads.map(ad => ({
            ...ad,
            likes: ad.likes + Math.floor(Math.random() * 3), // 0–2
            comments: ad.comments + (Math.random() < 0.25 ? 1 : 0),
            clicks: ad.clicks + Math.floor(Math.random() * 2), // 0–1
          })),
        })),
      );
    }, 10000); // 10 sec
  ...
  // ---- Brand-level stats for new top row ----
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 animate-slide-in-up w-full max-w-7xl mx-auto font-inter">
      <div className="bg-white/95 border border-gray-200 rounded-2xl shadow-sm p-6 md:p-8">
        ...
        {/* NEW TOP STRIP – brand overview (instead of Total Spend/Clicks etc.) */}
        ...
        {/* ---- LIVE BRAND CAMPAIGNS (Puma / Nike) ---- */}
        ...
        {/* --- MAIN CONTENT: YOUR SAVED ADAPP CAMPAIGNS (DB) --- */}
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* RIGHT: Details for selected DB campaign */}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between gap-3 mb-2">
                          <h2 className="text-2xl font-bold text-gray-900 leading-tight line-clamp-2">
                            {selectedCampaign.headline || selectedCampaign.name}
                          </h2>
