...
const CampaignSchema = new mongoose.Schema({
  ...
  clicks: { type: Number, default: 0 },  // kitne logon ne hamare tracking link par click kia
  purchases: { type: Number, default: 0 } // Shopify se kitni sales aa gayi
});
const Campaign = mongoose.model('Campaign', CampaignSchema);

// Instagram posts = public brand posts jo tum link karogi
...
app.get('/c/:campaignId', async (req, res) => {
  try {
    ...
    // Shopify URL me campaign id as query param add kar do, taaki
    // order ke time hame pata chale ki kis campaign se aaya.
...
app.post('/api/shopify/order-webhook', async (req, res) => {
  try {
    ...
    // 1) order.landing_site me hamara cid query param hoga
    //    example: /products/xyz?cid=675abc123
...
// ======================================================================
// 6A) HELPER – SCRAPE BASIC DATA FOR AN INSTAGRAM URL
// ======================================================================
async function fetchInstagramPostData(postUrl) {
  try {
    ...
    // kabhi kabhi yeh patterns kaam karte hain, kabhi nahi
    ...
    console.warn('⚠️ fetchInstagramPostData failed:', err.message);
    return {
      imageUrl: null,
      caption: '',
      likes: null,
...
// ======================================================================
// 6C) INSTAGRAM POSTS PER CAMPAIGN (PERMANENT SAVE)
// ======================================================================

// helper: build one InstagramPost payload from URL
async function buildInstagramPostRecord(url) {
  // TRY: scrape basic data
  const meta = await fetchInstagramPostData(url);

  // agar Instagram allow kare to real values milengi, nahi to null
  ...
  return {
    ...
    likes,
    comments,
    // ab yahan koi fake maths nahi, simple 0
    clicks: 0,
  };
}

// Add new Instagram post to a campaign
...
// Get all IG posts for a campaign
...
// Delete IG post from a campaign
